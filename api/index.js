...
var serverlessHandler_exports = {};
__export(serverlessHandler_exports, {
  default: () => serverlessHandler_default
});
module.exports = __toCommonJS(serverlessHandler_exports);
...
// src/server/controllers/authController.ts
var JWT_SECRET = process.env.JWT_SECRET || "super_secret_jwt_key_trao_prep_kit_2026";
  ...
  static async register(req, res) {
    try {
      ...
      if (isMongoConnected()) {
        ...
        const token = import_jsonwebtoken.default.sign({ id: user._id.toString(), email: user.email }, JWT_SECRET, {
          expiresIn: "7d"
        });
        res.status(201).json({
        ...
        const token = import_jsonwebtoken.default.sign({ id: user.id, email: user.email }, JWT_SECRET, {
          expiresIn: "7d"
        });
        res.status(201).json({
          message: "Account created successfully",
...
var JWT_SECRET2 = process.env.JWT_SECRET || "super_secret_jwt_key_trao_prep_kit_2026";
...
// src/server/routes/kitRoutes.ts
var import_express2 = require("express");

// src/server/controllers/kitController.ts
...
var LLMClient = class {
  ...
  constructor() {
    ...
    if (process.env.GROQ_API_KEY) {
      this.groqClient = new groq_sdk_default({ apiKey: process.env.GROQ_API_KEY });
    }
  ...
  async complete(messages, options = { temperature: 0.2, responseFormat: "json" }) {
    ...
    for (let attempt = 0; attempt <= maxRetries; attempt++) {
      try {
        ...
        const isRateLimit = err?.status === 429 || err?.message?.includes("429") || err?.message?.toLowerCase()?.includes("rate limit") || err?.message?.toLowerCase()?.includes("quota") || err?.message?.toLowerCase()?.includes("resource exhausted");
        const isTransient = err?.status >= 500 || err?.code === "ECONNRESET" || err?.code === "ETIMEDOUT" || err?.message?.toLowerCase()?.includes("overloaded");
  ...
  async callGemini(messages, options) {
    ...
    const userMsgs = messages.filter((m2) => m2.role !== "system");
    const contents = userMsgs.map((m2) => ({
    ...
    if (this.geminiClient) {
      const modelInstance = this.geminiClient.getGenerativeModel({
        model: modelName,
        systemInstruction: systemMsg ? { role: "system", parts: [{ text: systemMsg }] } : void 0,
        generationConfig: {
          temperature: options.temperature ?? 0.2,
          maxOutputTokens: options.maxTokens ?? 4096,
          responseMimeType: options.responseFormat === "json" ? "application/json" : "text/plain"
  ...
  async callGroq(messages, options) {
    ...
    const groqMessages = messages.map((m2) => ({
      role: m2.role,
      content: m2.content
    }));
  ...
  parseJSON(raw) {
    ...
    try {
      ...
      throw new Error(`Failed to parse LLM JSON output. Raw snippet: ${raw.substring(0, 200)}... Error: ${primaryErr.message}`);
    }
  }
  /**
   ...
   */
  generateMockResponse(messages, options) {
    const userPrompt = messages.map((m2) => m2.content).join("\n").toLowerCase();
    ...
    if (userPrompt.includes("extract the structured role details") || userPrompt.includes("job description analyst") || userPrompt.includes("parse the following job description") || userPrompt.includes("extract requirements")) {
      return JSON.stringify({
...
var MEDIUM_PRIORITY_KEYWORDS = ["team", "mission", "tech-stack", "working-at", "people", "press"];
...
var CompanyCrawler = class {
  userAgent = "TraoPrepBot/1.0 (+https://github.com/trao/interview-prep)";
  /**
   * Validates if a URL is safe to crawl (rejects loopback/private addresses in production)
   */
  isUrlSafe(urlStr, allowLocalhost = true) {
    try {
      const parsed = new import_url.URL(urlStr);
      if (!["http:", "https:"].includes(parsed.protocol)) {
        return false;
      }
      const hostname = parsed.hostname.toLowerCase();
      const isLoopback = hostname === "localhost" || hostname === "127.0.0.1" || hostname === "::1" || hostname.startsWith("192.168.") || hostname.startsWith("10.") || hostname.startsWith("172.16.");
  ...
  async crawl(startUrl, options = {}) {
    ...
    let parsedStart;
    try {
      parsedStart = new import_url.URL(startUrl);
    } catch (e2) {
    ...
    try {
      ...
      if (robotsRes.status === 200 && typeof robotsRes.data === "string") {
        robots = (0, import_robots_parser.default)(robotsUrl, robotsRes.data);
      }
    } catch {
    }
    ...
    while (discoveredLinks.length > 0 && result.pages.length < maxPages) {
      discoveredLinks.sort((a2, b2) => b2.score - a2.score);
      ...
      let response;
      try {
        response = await import_axios2.default.get(current.url, {
          headers: {
      ...
      if (!result.companyName) {
        const ogSiteName = $2('meta[property="og:site_name"]').attr("content");
        const titleText = $2("title").text().trim();
        if (ogSiteName) {
      ...
      $2("a[href]").each((_2, el) => {
        ...
        const anchor = $2(el).text().trim();
        if (!href) return;
        try {
          const resolved = new import_url.URL(href, current.url);
      ...
      $2("script, style, noscript, svg, nav, footer, iframe, header, form").remove();
      const headings = [];
      $2("h1, h2, h3").each((_2, el) => {
        const h2 = $2(el).text().trim();
        if (h2 && h2.length > 2 && h2.length < 150) {
          headings.push(h2);
        }
      });
      let bodyText = $2("body").text() || $2.text();
      bodyText = bodyText.replace(/\r\n/g, "\n").replace(/[ \t]+/g, " ").replace(/\n\s*\n+/g, "\n\n").trim();
      if (bodyText.length > 8e3) {
        bodyText = bodyText.substring(0, 8e3) + "... [truncated]";
      }
      ...
      result.pages.push({
        url: current.url,
        title: $2("title").text().trim() || current.anchor,
        cleanText: bodyText,
...
var companyCrawler = new CompanyCrawler();

// src/core/research/companyResearch.ts
var CompanyResearcher = class {
  constructor(llm) {
    this.llm = llm;
  }
  llm;
   ...
   */
  async research(companyUrl, crawlResult, companyHint) {
    ...
    const hiringPages = pages.filter((p2) => p2.isHiringPage);
    const combinedContext = pages.map((p2) => `--- PAGE: ${p2.title} (${p2.url}) ---
Headings: ${p2.headings.join(" | ")}
${p2.cleanText.substring(0, 3e3)}`).join("\n\n");
...
Return ONLY valid JSON matching this exact structure:
{
  ...
  "public_discussion_notes": "..."
    ...
    try {
      ...
      const parsed = this.llm.parseJSON(responseText);
      return {
        brief: {
        ...
        hiringProcessSummary: parsed.hiring_process_notes || (hiringPages.length > 0 ? "Identified dedicated career/hiring pages with role outlines and team engineering practices." : "No dedicated public interview or hiring process pages discovered during site crawl."),
        publicDiscussionSummary: parsed.public_discussion_notes || "Public interview discussions for this specific domain were checked; no verified public interview threads found on the company domain.",
        sources: sources.length > 0 ? sources : [companyUrl]
      };
    } catch (err) {
      return {
...
// src/core/extraction/requirementExtractor.ts
var RequirementExtractor = class {
  constructor(llm) {
    this.llm = llm;
  }
  llm;
  async extract(jdText) {
    ...
    const lineCount = trimmed.split("\n").filter((l2) => l2.trim().length > 0).length;
...
Return ONLY valid JSON with this exact schema:
{
  ...
  "requirements": [
    ...
    try {
      ...
      const validKinds = ["technical", "behavioural", "domain"];
      const validPriorities = ["must", "nice"];
      const sanitizedRequirements = (parsed.requirements || []).map((req, idx) => {
        const kind2 = validKinds.includes(req.kind) ? req.kind : "technical";
        const priority = validPriorities.includes(req.priority) ? req.priority : "must";
        return {
          id: `r${idx + 1}`,
          text: req.text.trim(),
          kind: kind2,
      ...
      const role = {
        ...
        responsibilities: parsed.responsibilities && parsed.responsibilities.length > 0 ? parsed.responsibilities : ["Execute responsibilities outlined in the job description."],
...
// src/core/generation/questionGenerator.ts
var QuestionGenerator = class {
   ...
   */
  async generateQuestionsForCategory(category, requirements, companyBrief, hiringNotes, existingQuestionCount = 0) {
    let targetReqs;
    if (category === "technical") {
      targetReqs = requirements.filter((r2) => r2.kind === "technical" || r2.kind === "domain");
    } else if (category === "behavioural") {
      targetReqs = requirements.filter((r2) => r2.kind === "behavioural");
...
${targetReqs.map((r2) => `[ID: ${r2.id}] (${r2.priority.toUpperCase()} - ${r2.kind}): ${r2.text}`).join("\n")}
...
[
  {
    ...
    try {
      ...
      const validRids = new Set(requirements.map((r2) => r2.id));
      return questionsList.map((q2, idx) => {
        const rids = (q2.requirement_ids || []).filter((id) => validRids.has(id));
        if (rids.length === 0) {
          rids.push(targetReqs[0]?.id || requirements[0]?.id || "r1");
        }
        const diff = [1, 2, 3].includes(q2.difficulty) ? q2.difficulty : 2;
        return {
          id: `q${existingQuestionCount + idx + 1}`,
          requirement_ids: rids,
          category,
          prompt: q2.prompt?.trim() || `Core ${category} interview assessment`,
          answer_outline: q2.answer_outline?.trim() || "Provide key principles, practical examples, and trade-off evaluation.",
          difficulty: diff
        };
      });
    } catch {
   ...
   */
  async generateGapClosingQuestions(uncoveredRequirements, companyBrief, startIndex) {
...
${uncoveredRequirements.map((r2) => `[ID: ${r2.id}] (${r2.priority.toUpperCase()} - ${r2.kind}): ${r2.text}`).join("\n")}
...
[
  {
    ...
    try {
      ...
      return uncoveredRequirements.map((req, idx) => {
        ...
        const cat = validCategories.includes(item?.category) ? item.category : req.kind === "behavioural" ? "behavioural" : "technical";
        const diff = [1, 2, 3].includes(item?.difficulty) ? item.difficulty : 2;
        return {
   ...
   */
  async generateFlashcards(requirements, questions) {
    const prompt = `Generate 4 to 8 high-impact interview flashcards based on the following requirements and questions.

Requirements:
${requirements.map((r2) => `[${r2.id}]: ${r2.text}`).join("\n")}

Selected Questions:
${questions.slice(0, 5).map((q2) => `[${q2.id}]: ${q2.prompt}`).join("\n")}
...
[
  {
    ...
    try {
      ...
      if (cards.length < 4 && requirements.length > 0) {
        for (let i2 = cards.length; i2 < Math.min(6, Math.max(4, requirements.length)); i2++) {
          const req = requirements[i2 % requirements.length];
          cards.push({
            id: `f${i2 + 1}`,
...
// src/core/coverage/coverageChecker.ts
var CoverageChecker = class _CoverageChecker {
  /**
  ...
  static analyze(requirements, questions) {
    ...
    for (const q2 of questions) {
      for (const rid of q2.requirement_ids) {
        coveredSet.add(rid);
      }
    }
    const uncoveredRequirementIds = [];
    const uncoveredMustHaves = [];
    const uncoveredNiceToHaves = [];
    ...
    const coverageRatio = requirements.length > 0 ? coveredCount / requirements.length : 1;
    return {
   ...
   */
  static async runCoverageLoop(requirements, initialQuestions, companyBrief, questionGenerator, maxPasses = 3, onProgress) {
    let currentQuestions = [...initialQuestions];
    let passes = 1;
    while (passes <= maxPasses) {
      const analysis = _CoverageChecker.analyze(requirements, currentQuestions);
      if (onProgress) {
    ...
    const finalAnalysis = _CoverageChecker.analyze(requirements, currentQuestions);
    return {
      questions: currentQuestions,
      coverage: {
...
var DeterministicScheduler = class _DeterministicScheduler {
  /**
   * Allocates topics and questions across exactly `daysAvailable` days.
   * Pure deterministic application logic - NO LLM.
   */
  static schedule(daysAvailable, questions, requirements) {
    const N2 = Math.max(1, Math.round(daysAvailable));
    if (questions.length === 0) {
      const emptyDays = [];
      for (let d2 = 1; d2 <= N2; d2++) {
        emptyDays.push({
          day: d2,
          focus: d2 === N2 ? "Final Review & Preparation" : "Foundational Domain Study",
          question_ids: [],
          minutes: 45
        });
      }
      return { days_available: N2, days: emptyDays };
    }
    const mustReqIds = new Set(
      requirements.filter((r2) => r2.priority === "must").map((r2) => r2.id)
    );
    const scoredQuestions = questions.map((q2) => {
      let score = 0;
      const coversMust = q2.requirement_ids.some((rid) => mustReqIds.has(rid));
      if (coversMust) score += 1e3;
      score += q2.difficulty * 100;
      if (q2.category === "system-design") score += 40;
      else if (q2.category === "technical") score += 30;
      else if (q2.category === "behavioural") score += 20;
      else if (q2.category === "company-fit") score += 10;
      return { question: q2, score, coversMust };
    });
    scoredQuestions.sort((a2, b2) => b2.score - a2.score);
    const mustCoveredBySelected = /* @__PURE__ */ new Set();
    ...
    const dayBuckets = Array.from({ length: N2 }, () => []);
    if (N2 === 1) {
      const allQids = scoredQuestions.map((sq) => sq.question.id);
      dayBuckets[0] = allQids;
    } else if (N2 >= questions.length) {
      scoredQuestions.forEach((sq, idx) => {
        dayBuckets[idx].push(sq.question.id);
      });
      let pointer = 0;
      for (let d2 = questions.length; d2 < N2; d2++) {
        const repeatQuestion = scoredQuestions[pointer % scoredQuestions.length];
        dayBuckets[d2].push(repeatQuestion.question.id);
        pointer++;
      }
    } else {
      scoredQuestions.forEach((sq, idx) => {
        const targetDay = Math.min(N2 - 1, Math.floor(idx / scoredQuestions.length * N2));
        dayBuckets[targetDay].push(sq.question.id);
      });
      for (let d2 = 0; d2 < N2; d2++) {
        if (dayBuckets[d2].length === 0) {
          const donorDay = d2 > 0 ? d2 - 1 : Math.min(d2 + 1, N2 - 1);
          if (dayBuckets[donorDay].length > 1) {
            const borrowed = dayBuckets[donorDay].pop();
            dayBuckets[d2].push(borrowed);
          } else {
            dayBuckets[d2].push(scoredQuestions[d2 % scoredQuestions.length].question.id);
          }
        }
      }
    }
    ...
    const qMap = new Map(questions.map((q2) => [q2.id, q2]));
    const days = dayBuckets.map((qids, idx) => {
      const dayNum = idx + 1;
      const dayQuestions = qids.map((id) => qMap.get(id)).filter(Boolean);
      let totalMinutes = 15;
      for (const q2 of dayQuestions) {
        if (q2.difficulty === 3) totalMinutes += 30;
        else if (q2.difficulty === 2) totalMinutes += 20;
        else totalMinutes += 15;
      }
      const integerMinutes = Math.min(180, Math.max(45, Math.round(totalMinutes)));
      const focus = _DeterministicScheduler.determineDailyFocus(dayNum, N2, dayQuestions);
      return {
        day: dayNum,
        focus,
   ...
   */
  static determineDailyFocus(day, totalDays, questions) {
    if (totalDays === 1) {
    ...
    const categories = new Set(questions.map((q2) => q2.category));
...
// src/core/validator/kitValidator.ts
var import_zod = require("zod");
var RequirementKindSchema = import_zod.z.enum(["technical", "behavioural", "domain"]);
var RequirementPrioritySchema = import_zod.z.enum(["must", "nice"]);
...
var QuestionCategorySchema = import_zod.z.enum(["technical", "behavioural", "system-design", "company-fit"]);
var QuestionDifficultySchema = import_zod.z.union([import_zod.z.literal(1), import_zod.z.literal(2), import_zod.z.literal(3)]);
var QuestionSchema = import_zod.z.object({
  id: import_zod.z.string().min(1),
  requirement_ids: import_zod.z.array(import_zod.z.string()),
...
var ScheduleDaySchema = import_zod.z.object({
  day: import_zod.z.number().int().min(1),
  focus: import_zod.z.string().min(1),
  question_ids: import_zod.z.array(import_zod.z.string()),
  minutes: import_zod.z.number().int().min(1)
});
var ScheduleSchema = import_zod.z.object({
  days_available: import_zod.z.number().int().min(1),
...
var CoverageSchema = import_zod.z.object({
  ...
  passes: import_zod.z.number().int().min(1)
});
var SourceSchema = import_zod.z.object({
  ...
  pages_used: import_zod.z.array(import_zod.z.string())
});
...
var KitSchema = import_zod.z.object({
  ...
  const questionIdSet = new Set(data.questions.map((q2) => q2.id));
  for (let i2 = 0; i2 < data.schedule.days.length; i2++) {
    const day = data.schedule.days[i2];
    for (const qid of day.question_ids) {
      if (!questionIdSet.has(qid)) {
        ctx.addIssue({
          code: import_zod.z.ZodIssueCode.custom,
          message: `Schedule day ${day.day} refers to non-existent question ID "${qid}"`,
          path: ["schedule", "days", i2, "question_ids"]
        });
      }
    }
  }
  const requirementIdSet = new Set(data.role.requirements.map((r2) => r2.id));
  for (let i2 = 0; i2 < data.questions.length; i2++) {
    const q2 = data.questions[i2];
    for (const rid of q2.requirement_ids) {
      if (!requirementIdSet.has(rid)) {
        ctx.addIssue({
          code: import_zod.z.ZodIssueCode.custom,
          message: `Question "${q2.id}" refers to non-existent requirement ID "${rid}"`,
          path: ["questions", i2, "requirement_ids"]
        });
      }
    }
  }
  const scheduledQuestionIds = new Set(data.schedule.days.flatMap((d2) => d2.question_ids));
  const scheduledRequirementIds = /* @__PURE__ */ new Set();
  for (const q2 of data.questions) {
    if (scheduledQuestionIds.has(q2.id)) {
      for (const rid of q2.requirement_ids) {
        scheduledRequirementIds.add(rid);
      }
    }
  }
  const mustHaveReqs = data.role.requirements.filter((r2) => r2.priority === "must");
...
function validateKit(obj) {
  ...
  return {
    success: false,
    errors: result.error.errors.map((e2) => `${e2.path.join(".")}: ${e2.message}`)
  };
}
var BatchInputCaseSchema = import_zod.z.object({
  id: import_zod.z.string().min(1),
  jd: import_zod.z.string().min(1),
  company_url: import_zod.z.string().min(1),
  days: import_zod.z.number().int().min(1)
});
var BatchInputSchema = import_zod.z.array(BatchInputCaseSchema);
var BatchOutputSchema = import_zod.z.object({
  version: import_zod.z.literal("1.0"),
  generated_at: import_zod.z.string(),
  kits: import_zod.z.array(
    import_zod.z.object({
      id: import_zod.z.string(),
      status: import_zod.z.enum(["ok", "failed"]),
      kit: import_zod.z.record(import_zod.z.any()).nullable(),
...
// src/core/pipeline/generateKit.ts
var GenerationPipeline = class {
  llm;
  crawler;
  researcher;
  extractor;
  questionGenerator;
  constructor(llmClientInstance) {
  ...
  async run(options) {
    ...
    const warnings = [];
    const totalSteps = 11;
    const progress = (step, name, detail) => {
...
var StateManager = class {
  ...
  static fromKit(kit) {
    ...
    return {
      ...
      questions: kit.questions.map((q2) => ({
        ...q2,
        _meta: { ...defaultMeta }
      })),
      flashcards: kit.flashcards.map((f2) => ({
        ...f2,
        _meta: { ...defaultMeta }
      }))
    };
  ...
  static toCleanKit(enhanced) {
    ...
    const cleanQuestions = enhanced.questions.map((q2) => ({
      id: q2.id,
      requirement_ids: q2.requirement_ids,
      category: q2.category,
      prompt: q2.prompt,
      answer_outline: q2.answer_outline,
      difficulty: q2.difficulty
    }));
    const cleanFlashcards = enhanced.flashcards.map((f2) => ({
      id: f2.id,
      front: f2.front,
      back: f2.back,
      requirement_ids: f2.requirement_ids
    }));
    return {
   ...
   */
  static editQuestion(kit, questionId, updates) {
    const questions = kit.questions.map((q2) => {
      if (q2.id !== questionId) return q2;
      return {
        ...q2,
        ...updates,
        _meta: {
          origin: q2._meta?.origin || "generated",
          status: "edited",
   ...
   */
  static togglePinQuestion(kit, questionId) {
    const questions = kit.questions.map((q2) => {
      if (q2.id !== questionId) return q2;
      const isPinned = q2._meta?.status === "pinned";
      return {
        ...q2,
        _meta: {
          origin: q2._meta?.origin || "generated",
   ...
   */
  static addManualQuestion(kit, newQuestion) {
   ...
   * Deletes a question
   */
  static deleteQuestion(kit, questionId) {
    const questions = kit.questions.filter((q2) => q2.id !== questionId);
    const days = kit.schedule.days.map((d2) => ({
      ...d2,
      question_ids: d2.question_ids.filter((id) => id !== questionId)
    }));
    return {
      ...kit,
      questions,
   ...
   */
  static async regenerateCategory(kit, targetCategory, generator, hiringNotes = "") {
    const preservedInTarget = [];
    const otherCategoryQuestions = [];
    for (const q2 of kit.questions) {
      if (q2.category === targetCategory) {
        const isPinned = q2._meta?.status === "pinned";
        const isEdited = q2._meta?.status === "edited";
        const isManual = q2._meta?.origin === "manual";
        if (isPinned || isEdited || isManual) {
          preservedInTarget.push(q2);
        }
      } else {
        otherCategoryQuestions.push(q2);
      }
    }
    const maxExistingIndex = kit.questions.reduce((max, q2) => {
      const match = q2.id.match(/^q(\d+)$/);
    ...
    const enhancedNewQuestions = newlyGenerated.map((q2) => ({
      ...q2,
      _meta: {
        origin: "generated",
        status: "unmodified",
    ...
    const cleanQs = allQuestions.map((q2) => ({
      id: q2.id,
      requirement_ids: q2.requirement_ids,
      category: q2.category,
      prompt: q2.prompt,
   ...
   */
  static async regenerateCompanyBrief(kit, researcher, crawlResult) {
   ...
   */
  static regenerateSchedule(kit, daysAvailable) {
    const days = daysAvailable ?? kit.schedule.days_available;
    const cleanQuestions = kit.questions.map((q2) => ({
      id: q2.id,
      requirement_ids: q2.requirement_ids,
      category: q2.category,
      prompt: q2.prompt,
...
// src/server/models/KitModel.ts
var import_mongoose3 = __toESM(require("mongoose"));
var KitSchema2 = new import_mongoose3.Schema(
  {
    userId: { type: String, required: true, index: true },
    kit: { type: import_mongoose3.Schema.Types.Mixed, required: true },
    practiceStates: { type: import_mongoose3.Schema.Types.Mixed, default: {} }
  },
  { timestamps: true }
);
var KitMongoModel = import_mongoose3.default.models.Kit || import_mongoose3.default.model("Kit", KitSchema2);
var memoryKits = /* @__PURE__ */ new Map();

// src/server/controllers/kitController.ts
function isMongoConnected2() {
...
var KitController = class {
   ...
   */
  static async generateKit(req, res) {
    try {
    ...
    try {
      ...
      if (isMongoConnected2()) {
        ...
        res.json({
          kits: docs.map((d2) => ({
            id: d2._id.toString(),
            company: d2.kit.source.company,
            role: d2.kit.role.title,
            days: d2.kit.schedule.days_available,
            questionCount: d2.kit.questions.length,
            flashcardCount: d2.kit.flashcards.length,
   ...
   */
  static async regenerateSection(req, res) {
    try {
      const userId = req.user?.id;
      const { id } = req.params;
      ...
      if (section === "company_brief") {
        ...
        const crawl = await crawler.crawl(currentKit.source.company_url, { maxPages: 3, timeoutMs: 5e3 });
        ...
        updatedKit = await StateManager.regenerateCategory(currentKit, section, generator);
      } else {
   ...
   * Deletes a kit
   */
  static async deleteKit(req, res) {
    try {
      const userId = req.user?.id;
      const { id } = req.params;
      if (isMongoConnected2()) {
   ...
   */
  static async evaluateMockAnswer(req, res) {
    try {
      const { id } = req.params;
      ...
      const targetQuestion = kit.questions.find((q2) => q2.id === questionId);
      if (!targetQuestion) {
...
Return ONLY valid JSON:
{
  ...
  "key_tips": ["...", "...", "..."]
      ...
      const parsed = llm.parseJSON(response);
      res.json({
        questionId,
        evaluation: parsed
      });
...
router2.post("/generate", KitController.generateKit);
router2.get("/", KitController.listKits);
router2.get("/:id", KitController.getKit);
router2.put("/:id", KitController.updateKit);
router2.post("/:id/regenerate-section", KitController.regenerateSection);
router2.delete("/:id", KitController.deleteKit);
router2.post("/:id/mock-interview", KitController.evaluateMockAnswer);
var kitRoutes_default = router2;

// src/server/routes/practiceRoutes.ts
var import_express3 = require("express");

// src/server/controllers/practiceController.ts
var import_mongoose5 = __toESM(require("mongoose"));

// src/core/practice/practiceEngine.ts
var PracticeEngine = class {
  /**
   * Updates or creates state for a reviewed flashcard
   */
  static recordReview(currentState, cardId, rating) {
   ...
   */
  static getNextPracticeQueue(flashcards, stateMap) {
    const cardsWithScore = flashcards.map((card) => {
      ...
      const timeElapsedSec = state.last_reviewed_at ? (Date.now() - new Date(state.last_reviewed_at).getTime()) / 1e3 : 0;
  ...
  static computeProgress(flashcards, stateMap) {
    ...
    const coverage_percentage = total_cards > 0 ? Math.round(reviewed_cards / total_cards * 100) : 0;
    return {
      total_cards,
      reviewed_cards,
      mastered_count,
...
var PracticeController = class {
  ...
  static async recordCardReview(req, res) {
    try {
      ...
      if (isMongoConnected3()) {
        ...
        const stateMap = new Map(Object.entries(item.practiceStates || {}));
        const updatedMap = PracticeEngine.recordReview(stateMap, cardId, confRating);
        const updatedObj = Object.fromEntries(updatedMap);
   ...
   */
  static async getPracticeQueue(req, res) {
    try {
      const userId = req.user?.id;
      const { kitId } = req.params;
      let flashcards = [];
      ...
      const stateMap = new Map(Object.entries(practiceStates));
...
router3.post("/:kitId/record", PracticeController.recordCardReview);
router3.get("/:kitId/queue", PracticeController.getPracticeQueue);
var practiceRoutes_default = router3;
...
function createApp() {
  ...
  app2.use(import_express4.default.json({ limit: "10mb" }));
  app2.use(import_express4.default.urlencoded({ extended: true, limit: "10mb" }));
  ...
  app2.use("/api/kits", kitRoutes_default);
  app2.use("/kits", kitRoutes_default);
  app2.use("/api/practice", practiceRoutes_default);
  app2.use("/practice", practiceRoutes_default);
  app2.use((_req, res) => {
    res.status(404).json({
      error: "NOT_FOUND",
...
var serverlessHandler_default = handler;
...
@google/generative-ai/dist/index.mjs:
   ...
   *)
*/
module.exports = serverlessHandler_default; module.exports.default = serverlessHandler_default;
